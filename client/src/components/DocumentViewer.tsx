...
export function DocumentViewer({ document, isOpen, onClose }: DocumentViewerProps) {
  ...
  const loadDocument = async () => {
    ...
    try {
      ...
      if (isLegacyDoc) {
        await loadLegacyDocDocument();
      } else if (isModernDocx) {
  ...
  const loadLegacyDocDocument = async () => {
    ...
    try {
      ...
      if (wordContainerRef.current) {
        wordContainerRef.current.innerHTML = result.value;
        setWordContent('rendered');
      }
    } catch (err) {
      console.error('Ошибка при загрузке .doc документа:', err);
      setError('legacy_doc');
    } finally {
      setLoading(false);
    }
  ...
  const renderContent = () => {
    ...
    if (isDwg || isOtherCAD) {
      return (
        <div className="flex flex-col items-center justify-center p-8 text-center">
          <div className="mb-6 p-8 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
            <svg className="mx-auto h-24 w-24 text-blue-500 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 3v8h8" />
            </svg>
            <p className="text-blue-800 dark:text-blue-200 font-semibold mb-2 text-lg">
              {isDwg ? 'Чертеж AutoCAD (.dwg)' : `Файл CAD (${document.fileName.split('.').pop()?.toUpperCase()})`}
            </p>
            <p className="text-blue-700 dark:text-blue-300 text-sm mb-4 max-w-md">
              Для просмотра чертежей и 3D-моделей требуются специализированные программы:
              AutoCAD, AutoCAD Viewer, DWG TrueView, FreeCAD или другие CAD приложения.
            </p>
            <div className="text-left text-sm text-blue-600 dark:text-blue-400 mb-4 max-w-md mx-auto">
              <p className="font-medium mb-1">Рекомендуемые программы:</p>
              <ul className="list-disc list-inside space-y-1">
                <li>AutoCAD Viewer (бесплатно)</li>
                <li>DWG TrueView (бесплатно)</li>
                <li>FreeCAD (бесплатно, open-source)</li>
              </ul>
            </div>
          </div>
          <Button onClick={handleDownload} size="lg">
    ...
    if (isArchive) {
      return (
        <div className="flex flex-col items-center justify-center p-8 text-center">
          <div className="mb-6 p-8 bg-purple-50 dark:bg-purple-900/20 rounded-lg border border-purple-200 dark:border-purple-800">
            <svg className="mx-auto h-24 w-24 text-purple-500 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M5 19a2 2 0 01-2-2V7a2 2 0 012-2h4l2 2h4a2 2 0 012 2v1M5 19h14a2 2 0 002-2v-5a2 2 0 00-2-2H9a2 2 0 00-2 2v5a2 2 0 01-2 2z" />
            </svg>
            <p className="text-purple-800 dark:text-purple-200 font-semibold mb-2 text-lg">
              Архив ({document.fileName.split('.').pop()?.toUpperCase()})
            </p>
            <p className="text-purple-700 dark:text-purple-300 text-sm">
              Скачайте архив и распакуйте его с помощью архиватора (WinRAR, 7-Zip, и др.)
            </p>
          </div>
          <Button onClick={handleDownload} size="lg">
            <Download className="mr-2 h-5 w-5" />
    ...
    return (
      <div className="flex flex-col items-center justify-center p-8 text-center">
        <div className="mb-6 p-8 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
          ...
          <p className="text-muted-foreground text-sm mb-1">
            Тип файла: {mimeType || 'Неизвестно'}
          </p>
          <p className="text-muted-foreground text-sm">
            Предварительный просмотр недоступен для этого формата
          </p>
        </div>
        <Button onClick={handleDownload} size="lg">
          <Download className="mr-2 h-5 w-5" />
